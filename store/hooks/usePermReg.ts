...
import {
  startPermRegForm,
  submitPermRegDocuments,
  createPermRegPayment,
  submitPermRegApplication,
  loadExistingPermRegData,
} from "../actions/permregActions";
import {
  ...
  clearNOCError,
  resetNOCForm,
  selectPERMREGFormState,
  selectPERMREGCurrentStep,
  selectPERMREGSubmitSuccess,
  selectPERMREGDocuments,
  selectPERMREGPayment,
  selectPERMREGFormId,
  selectPERMREGLoading,
  selectPERMREGError,
  selectPERMREGFinalConsent,
  selectPERMREGSubmittedSteps,
  selectPERMREGNewRegistrationFormId,
  selectOverviewData,
  NOC_STEP_CONFIG,
  NOCDocuments,
  NOCPayment,
} from "../slices/permregSlice";

export const usePermReg = () => {
  const dispatch = useDispatch<AppDispatch>();

  // Selectors
  const formState = useSelector(selectPERMREGFormState);
  const currentStep = useSelector(selectPERMREGCurrentStep);
  const submitSuccess = useSelector(selectPERMREGSubmitSuccess);
  const documents = useSelector(selectPERMREGDocuments);
  const payment = useSelector(selectPERMREGPayment);
  const formId = useSelector(selectPERMREGFormId);
  const loading = useSelector(selectPERMREGLoading);
  const error = useSelector(selectPERMREGError);
  const finalConsent = useSelector(selectPERMREGFinalConsent);
  const submittedSteps = useSelector(selectPERMREGSubmittedSteps);
  const newRegistrationFormId = useSelector(selectPERMREGNewRegistrationFormId);
  const overviewData = useSelector(selectOverviewData);

  // Actions
  const startForm = useCallback(
    async (type: string = "PERM_REGISTRATION") => {
      try {
        const result = await dispatch(startPermRegForm(type)).unwrap();
        return result;
      } catch (err) {
        console.error("Failed to start perm reg form:", err);
        throw err;
      }
    },
    [dispatch],
    ...
    async (docs: Record<string, File>) => {
      try {
        const result = await dispatch(
          submitPermRegDocuments({ formId, documents: docs }),
        ).unwrap();
        return result;
      } catch (err) {
  ...
  const createPayment = useCallback(
    async (type: string) => {
      try {
        const result = await dispatch(
          createPermRegPayment({ permRegFormId: formId, type }),
        ).unwrap();
        return result;
      } catch (err) {
  ...
  const loadExistingData = useCallback(
    async (existingFormId: string) => {
      try {
        const result = await dispatch(
          loadExistingPermRegData(existingFormId),
        ).unwrap();
        return result;
      } catch (err) {
